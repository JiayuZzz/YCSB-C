...
def load(db,dbfilename,workload,resultfile="-1"):
    ...
    if resultfile=="-1":
        ...
        os.system("./ycsbc -db {0} -dbfilename {1} -threads 1 -P {2} -phase load > {3}".format(db,dbfilename,workload,resultfile))
...
def run(db,dbfilename,workload,resultfile="-1"):
    if resultfile=="-1":
        ...
        os.system("./ycsbc -db {0} -dbfilename {1} -threads 1 -P {2} -skipLoad run > {3}".format(db,dbfilename,workload,resultfile))
...
def both(db,dbfilename,workload,resultfile="-1"):
    if resultfile=="-1":
        ...
        os.system("./ycsbc -db {0} -dbfilename {1} -threads 1 -P {2} -skipLoad both > {3}".format(db,dbfilename,workload,resultfile))
